...
if test "$#" -eq 2; then
    ...
    find . -type f -size +40M -a ! -ipath '*.git/*' -a ! -ipath './digitalcorpora.org/*' -a ! -ipath './tika-collected-corpora/*' -a ! -ipath './__mujstest/*' -a ! -ipath './__unitest/*' -a ! -ipath './__bulktest/*' -a ! -iname '*.log' -a ! -iname '*.[tc]sv' -print0 | xargs -0 -n 1 "$0" SPLIT2
fi
